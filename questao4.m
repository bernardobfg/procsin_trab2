...
% Aplicar os filtros FIR e IIR nos sinais reamostrados
voz_filtered_parks = filter(fir_coeffs, 1, voz_resampled);
musica_filtered_parks = filter(fir_coeffs, 1, musica_resampled);
voz_filtered_cheby = filter(b, a, voz_resampled);
musica_filtered_cheby = filter(b, a, musica_resampled);
...
title('Espectrograma do Sinal de Voz Filtrado (Parks-McClellan)');
colorbar;
...
title('Espectrograma do Sinal de Música Filtrado (Parks-McClellan)');
colorbar;
...
title('Espectrograma do Sinal de Voz Filtrado (Chebyshev 1)');
colorbar;
...
title('Espectrograma do Sinal de Música Filtrado (Chebyshev 1)');
colorbar;
